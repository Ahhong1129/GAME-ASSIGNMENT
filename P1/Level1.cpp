...
void Level1::init()
{
	...
	rock_position[0].y = -800;

	rock_position[1].x = 250;
	rock_position[1].y = -1000;

	rock_position[2].x = 150;
	rock_position[2].y = -3900;

	rock_position[3].x = 250;
	rock_position[3].y = -1500;
	...
	for (int i = 0; i < 50; i++) {
		...
		for (int i = 0; i < particleList.size(); i++) {
			if (particleList[i]->isInUse == false) {
				particleList[i]->cut(rock_position[i],32,32);
				foundParticle = true;
			}
		}
		if (!foundParticle) {
			Particle* particle = new Particle();
			particle->cut(rock_position[i],32,32);
